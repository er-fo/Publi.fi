...
// Optional: Function to send you a notification when someone signs up
function sendAdminNotification(email, referralCode, usedReferralCode) {
  const adminEmail = "your-email@example.com"; // Replace with your email
  const subject = `New Publify Signup: ${email}`;
  
  const body = `
New signup details:
- Email: ${email}
- Personal Referral Code: ${referralCode}
- Used Referral Code: ${usedReferralCode || 'None'}
- Timestamp: ${new Date()}
